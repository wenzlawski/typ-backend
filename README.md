...
- 🎯 Entry points to execute your program using `python -m <typ_backend>` or `$ typ_backend` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/wenzlawski/typ-backend/branch/main/graph/badge.svg?token=typ-backend_token_here)](https://codecov.io/gh/wenzlawski/typ-backend)
[![CI](https://github.com/wenzlawski/typ-backend/actions/workflows/main.yml/badge.svg)](https://github.com/wenzlawski/typ-backend/actions/workflows/main.yml)

Awesome typ_backend created by wenzlawski

## Install

from source
```bash
git clone https://github.com/wenzlawski/typ-backend typ_backend
cd typ_backend
make install
```
...
```bash
pip install typ_backend
```

## Executing

```bash
$ typ_backend run --port 8080
```

or

```bash
python -m typ_backend run --port 8080
```

or

```bash
$ uvicorn typ_backend:app
```
...
```bash
❯ typ_backend --help
Usage: typ_backend [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ typ_backend create-user --help
Usage: typ_backend create-user [OPTIONS] USERNAME PASSWORD
...
```bash
typ_backend create-user admin admin --superuser
```
...
Run with `typ_backend run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `typ_backend create-user`.
...
-----------------------------------------------------
typ_backend/__init__.py              4      0   100%
typ_backend/app.py                  16      1    94%
typ_backend/cli.py                  21      0   100%
typ_backend/config.py                5      0   100%
typ_backend/db.py                   10      0   100%
typ_backend/models/__init__.py       0      0   100%
typ_backend/models/content.py       47      1    98%
typ_backend/routes/__init__.py      11      0   100%
typ_backend/routes/content.py       52     25    52%
typ_backend/routes/security.py      15      1    93%
typ_backend/routes/user.py          52     26    50%
typ_backend/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from typ_backend.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in typ_backend/default.toml.

### As environment variables
```bash
export typ_backend_KEY=value
export typ_backend_KEY="@int 42"
export typ_backend_KEY="@jinja {{ this.db.uri }}"
export typ_backend_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
typ_backend_ENV=production typ_backend run
```
